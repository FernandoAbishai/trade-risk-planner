...
class Handler(BaseHTTPRequestHandler):
    server_version = "TradeRiskPlanner/5.0"
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if not 0 < length <= 100_000: raise TradeInputError("Invalid request size.")
    ...
    def do_GET(self) -> None:
        ...
        if path=="/api/health": self._json(200,{"ok":True,"version":"5"}); return
        ...
        data=candidate.read_bytes(); mime,_=mimetypes.guess_type(str(candidate)); self.send_response(200); content_type=mime or "application/octet-stream"; content_type += "; charset=utf-8" if content_type.startswith("text/") or content_type=="application/javascript" else ""; self.send_header("Content-Type",content_type); self.send_header("Content-Length",str(len(data))); self.end_headers(); self.wfile.write(data)
    def log_message(self, fmt: str, *args) -> None: pass

def main() -> None:
    parser=argparse.ArgumentParser(description="Local Trade Risk Planner"); parser.add_argument("--host",default="127.0.0.1"); parser.add_argument("--port",type=int,default=8501); args=parser.parse_args(); server=ThreadingHTTPServer((args.host,args.port),Handler); print(f"Trade Risk Planner: http://{args.host}:{args.port}")
